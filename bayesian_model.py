...
## Load data
df = pd.read_csv("persistence_data_3.csv")
...
pan2 = np.repeat("pan2", len(df[df.year_month.between('2021-04','2023-03')]))
pos = np.repeat("pos", len(df[df.year_month.between('2023-04','2023-12')]))
...
pb3 = pos_b[:,51:75,:].mean(axis=1)
pb4 = pos_b[:,75:,:].mean(axis=1)
...
pc3 = pos_c[:,51:75,:].mean(axis=1)
pc4 = pos_c[:,75:,:].mean(axis=1)
